...
void stateMandarAlarma() {
  ...
  if(alarmas == 'A'){
    ...
    if(machine.executeOnce){
      ...
      Enviar_msj(numero, "Tome sus medicamentos.");
    }    
  ...
  case 'B':
    ...
    if(alarmas == 'B'){
      ...
      Enviar_msj(numero, "No se ha reconocido al usuario que intenta ingresar.");
    } 
      //sonido más agudo
      for (i=fMin;i<=fMax; i++){
  ...
  case 'C':
    ...
    if(alarmas == 'C'){
      ...
      Enviar_msj(numero, "Quedan pocas pastillas en el compartimento.");
    } 
